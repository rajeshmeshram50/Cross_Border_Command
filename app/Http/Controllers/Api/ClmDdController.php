...
class ClmDdController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $name = trim($data['name']);
        if (ClmDdDocument::where('client_id', $user->client_id)
            ->whereRaw('LOWER(name) = ?', [mb_strtolower($name)])
            ->exists()) {
            return response()->json([
                'status'  => false,
                'message' => "A due-diligence document named \"{$name}\" already exists. Pick a different name.",
            ], 409);
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (isset($data['name'])
            && ClmDdDocument::where('client_id', $user->client_id)
                ->where('id', '!=', $row->id)
                ->whereRaw('LOWER(name) = ?', [mb_strtolower($data['name'])])
                ->exists()) {
            return response()->json([
                'status'  => false,
                'message' => "Another due-diligence document named \"{$data['name']}\" already exists. Pick a different name.",
            ], 409);
        }
    ...
    private function nextCode(int $clientId): string
    {
        ...
        $codes = ClmDdDocument::where('client_id', $clientId)->pluck('code')->all();
        $maxN = 0;
        $taken = [];
        foreach ($codes as $c) {
