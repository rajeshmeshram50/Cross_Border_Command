        ...
        foreach ($items as $it) {
            ...
            foreach ($candidates as $row) {
                ...
                $matched[$row->id] = [
                    ...
                    // The segment that pulled this T&C in. The PDF groups the
                    // clauses under it, so a customer spanning several segments
                    // can tell which term belongs to which. Segment only — a
                    // clause applies to the whole segment, not to one line item,
                    // so naming products here read as narrower than the term is.
                    'segment'  => $seg->name,
                    'content'  => $row->content,
                ];
            }
