...
export const resolveFileUrl = (path?: string | null) => {
    ...
    const collapse = (u: string) => u.replace(/(?<!:)\/{2,}/g, "/");
    ...
    if (/^https?:\/\//i.test(raw)) {
        ...
        const fixed = raw.includes("/api/storage/")
            ? raw.replace("/api/storage/", "/storage/")
            : raw;
        return collapse(fixed);
    }
    ...
    return collapse(`${base}/storage/${p}`);
};
