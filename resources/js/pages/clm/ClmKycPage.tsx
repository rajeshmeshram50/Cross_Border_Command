...
type Kyc = { id: number; code: string; name: string; authority: string; expiry: string; status: 'active'|'inactive'; };
...
export default function ClmKycPage() {
  ...
  const reload = () => {
    ...
    Promise.all([
      ...
      .then(([k, a]) => {
        setRows(k.data.data ?? []); setCount(k.data.count ?? 0);
        setAuths(a.data.data ?? []);
      })
    ...
    <div className="clm-root">
      ...
      <div className="clm-head-card">
        <div className="clm-head-left">
          ...
          <div>
            ...
            <div className="clm-head-title">KYC Documents</div>
            <div className="clm-head-sub">Manage customer &amp; vendor KYC documents — onboarding identity, address, registration certificates.</div>
          </div>
        </div>
      ...
      <div className="clm-body-card">
        <div className="clm-tabs">
          <span className="clm-total-pill">Total KYC · <b>{count}</b></span>
          <div className="clm-search">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.2"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" /></svg>
            <input type="text" placeholder="Search by document name, ID or authority…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
          </div>
        </div>
            ...
            <tbody>
              ...
              {!loading && slice.length === 0 && (
                <tr><td colSpan={6} className="clm-empty">
                  <div className="clm-empty-ico"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0891b2" strokeWidth="2"><rect x="3" y="4" width="18" height="16" rx="2" /><circle cx="12" cy="10" r="3" /></svg></div>
                ...
                <tr key={r.id}>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <div className="clm-actions">
                      <button className="clm-act clm-act-edit" title="Edit" onClick={() => { setEditing(r); setModalOpen(true); }}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                      </button>
                      <button className="clm-act clm-act-del" title="Delete" onClick={() => setPendingDelete(r)}>
      ...
      {modalOpen && (
        <KycModal existing={editing} authorities={auths} nextCode={`KYC-${String(rows.length + 1).padStart(3, '0')}`} onClose={() => { setModalOpen(false); setEditing(null); }} onSave={(form) => onSave(form, editing?.id)} />
      )}
...
function KycModal(props: { existing: Kyc | null; authorities: Authority[]; nextCode: string; onClose: () => void; onSave: (f: { name: string; authority: string; expiry: string }) => void; }) {
      ...
      <div className="clm-modal" onClick={e => e.stopPropagation()}>
        <div className="clm-modal-head">
          <div className="clm-modal-head-left">
            <div className="clm-modal-head-ico">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2"><rect x="3" y="4" width="18" height="16" rx="2" /><circle cx="12" cy="10" r="3" /></svg>
            </div>
            <div>
        ...
        <div className="clm-modal-body">
          ...
          <div className="clm-field">
            ...
            <div className="clm-field-hint">Pulls from Authority Master — add new authorities from the Authority page first.</div>
