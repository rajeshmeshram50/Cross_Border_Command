...
export type Kyc = { id: number; code: string; name: string; authority: string; authority_names?: string; status: 'active'|'inactive'; in_use?: boolean; used_in?: string[] };
...
export default function ClmKycPage() {
  ...
  const reload = () => {
    ...
    api.get<{ status: boolean; data: Kyc[]; count: number; total?: number }>('/clm/kyc-documents', {
    ...
    <div className="clm-root" ref={rootRef}>
      ...
      {modalOpen && (
        <Suspense fallback={null}>
          <KycModal existing={editing} authorities={auths} nextCode={`KYC-${String(rows.length + 1).padStart(3, '0')}`} onClose={() => { setModalOpen(false); setEditing(null); }} onSave={(f) => onSave(f, editing?.id)} />
        </Suspense>
      )}
      <DeleteConfirmModal
        open={!!pendingDelete}
