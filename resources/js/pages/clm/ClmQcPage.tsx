      ...
      <div className="clm-head-card">
        <div className="clm-head-left">
          <div className="clm-head-ico">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><polyline points="9 11 12 14 22 4" /><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" /></svg>
          </div>
          <div>
            <div className="clm-crumb">Central CLM · Compliance &amp; Regulatory</div>
            <div className="clm-head-title">Quality &amp; Compliance Documents</div>
            <div className="clm-head-sub">QC certificates, testing standards, compliance verification records — ISO 9001, HACCP, GOTS, CE, WHO-GMP, BIS, RoHS, REACH.</div>
          </div>
        </div>
      ...
      <div className="clm-body-card">
        <div className="clm-tabs">
          ...
          <button className={`clm-tab ${tab === 'cert' ? 'active' : ''}`} onClick={() => { setTab('cert'); setPage(1); }}>
            Certificates <span className="clm-tab-count">{counts.cert}</span>
          </button>
          <button className={`clm-tab ${tab === 'comp' ? 'active' : ''}`} onClick={() => { setTab('comp'); setPage(1); }}>
            Compliance Docs <span className="clm-tab-count">{counts.comp}</span>
          </button>
          <div className="clm-search">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.2"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" /></svg>
            <input type="text" placeholder="Search by name, ID, purpose or authority…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
          </div>
        </div>
        <div className="clm-table-wrap">
          <table className="clm-table">
            <thead><tr>
              ...
              <th style={{ minWidth: 200 }}>QC DOCUMENT</th>
              <th>PURPOSE</th>
            ...
            <tbody>
              ...
              {!loading && slice.length === 0 && (
                <tr><td colSpan={7} className="clm-empty">
                  <div className="clm-empty-ico"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0891b2" strokeWidth="2"><polyline points="9 11 12 14 22 4" /><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" /></svg></div>
              ...
              {!loading && slice.map((r, i) => (
                <tr key={r.id}>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <span className="clm-badge clm-badge-teal">{r.issued_by}</span>
                  </td>
                  <td style={{ textAlign: 'center' }}>
