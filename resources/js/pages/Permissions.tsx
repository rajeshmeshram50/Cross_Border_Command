...
function SearchableUserSelect({ users, value, onChange, placeholder }: {
  users: ManagedUser[]; value: string; onChange: (v: string) => void; placeholder: string;
}) {
  ...
  return (
    <div className="relative min-w-[320px]" ref={ref}>
      <button onClick={() => setOpen(!open)} type="button"
        className="w-full flex items-center gap-2 px-3 py-2 rounded-xl border-[1.5px] border-border bg-surface text-[12.5px] text-left hover:border-primary/40 transition-all cursor-pointer">
        {selected ? (
          ...
          <div className="max-h-[280px] overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <div className="px-4 py-6 text-center text-[12px] text-muted">
                <Search size={16} className="mx-auto mb-2 opacity-40" />No users found{search ? ` for "${search}"` : ''}
              </div>
            ) : filtered.map(u => (
              <button key={u.id} type="button"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10.5px] text-muted truncate flex items-center gap-1.5">
                    <span>{u.email}</span>
                    {u.client && <><span className="text-border">·</span><span className="flex items-center gap-0.5"><Building2 size={9} />{u.client.org_name}</span></>}
                  </div>
                </div>
...
export default function Permissions() {
  ...
  useEffect(() => {
    Promise.all([api.get('/modules'), api.get('/permissions/users')]).then(([modRes, usersRes]) => {
      let mods: Module[] = modRes.data;
      if (!isSuperAdmin) mods = mods.filter(m => !['clients', 'plans', 'payments', 'settings'].includes(m.slug));
      setModules(mods);
  ...
  const loadUserPermissions = (userId: string) => {
    ...
    }).catch(() => { setMatrix({ ...freshMatrix }); }).finally(() => setLoadingPerms(false));
  };

  useEffect(() => { if (selectedUserId && modules.length > 0) loadUserPermissions(selectedUserId); }, [selectedUserId, modules.length]);
  ...
  const toggleColumn = (key: PermKey) => {
    ...
    const next = { ...matrix };
    modules.forEach(m => {
      if (!isSuperAdmin && myPerms) { const mod = modules.find(mm => mm.id === m.id); if (mod && myPerms[mod.slug] && !myPerms[mod.slug][key]) return; }
  ...
  const selectAll = (val: boolean) => {
    ...
    modules.forEach(m => {
      ...
      PERMS.forEach(p => {
        ...
        if (!isSuperAdmin && myPerms) { const mod = modules.find(mm => mm.id === m.id); next[m.id][p.key] = mod && myPerms[mod.slug] ? myPerms[mod.slug][p.key] : false; }
        else { next[m.id][p.key] = true; }
      });
    });
    setMatrix(next);
  };
  ...
  const handleSave = async () => {
    if (!selectedUserId) { toast.warning('Select User', 'Please select a client admin first'); return; }
    setSaving(true);
    try {
      const permissions = modules.map(m => {
        const p = matrix[m.id] || emptyPerms();
        return { module_id: m.id, can_view: !!p.can_view, can_add: !!p.can_add, can_edit: !!p.can_edit, can_delete: !!p.can_delete, can_export: !!p.can_export, can_import: !!p.can_import, can_approve: !!p.can_approve };
      });
  ...
  if (loading) return <div className="flex items-center justify-center py-20 text-muted text-[13px]"><Loader2 size={20} className="animate-spin mr-2" /> Loading...</div>;
      ...
      <div className="flex items-start justify-between flex-wrap gap-3 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] text-muted mt-0.5">
              {isSuperAdmin ? 'Assign module permissions to client admins' : 'Assign module permissions to branch users'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <SearchableUserSelect
            ...
            placeholder={isSuperAdmin ? 'Search client admin...' : 'Search branch user...'}
          />
      ...
      {selectedUserId && (
        <>
          ...
          <div className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden">
            {loadingPerms ? (
              <div className="flex items-center justify-center py-12 text-muted text-[12px]"><Loader2 size={18} className="animate-spin mr-2" /> Loading permissions...</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full" style={{ minWidth: 800 }}>
                  <thead>
                    <tr className="bg-sidebar">
                      <th className="text-left px-4 py-3 text-[9.5px] font-bold tracking-wider uppercase text-white/55" style={{ width: '30%' }}>Module</th>
                      {PERMS.map(p => (
                        ...
                        <tr key={mod.id} className="border-b border-border/30 hover:bg-primary/[.03] transition-colors group">
                          <td className="px-4 py-0">
                            <div className="flex items-center h-11 gap-2.5">
                              ...
                              {mod.is_default && <span className="text-[8px] font-bold px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-600 border border-emerald-200">DEFAULT</span>}
                              <button onClick={() => {
                                const next = { ...matrix }; next[mod.id] = {} as Record<PermKey, boolean>;
                                PERMS.forEach(p => { if (!isSuperAdmin && myPerms && myPerms[mod.slug] && !myPerms[mod.slug][p.key]) { next[mod.id][p.key] = false; } else { next[mod.id][p.key] = !allOn; } });
                                setMatrix(next);
                            ...
                            return (
                              <td key={p.key} className="text-center px-2 py-0">
                                <div className="flex items-center justify-center h-11">
                                  <input type="checkbox" checked={!!rowPerms[p.key]} onChange={() => toggle(mod.id, p.key)} disabled={!!disabled}
            ...
            <div className="px-4 py-3 border-t border-border/50 bg-surface-2 flex items-center justify-between">
              <span className="text-[11.5px] text-muted">
                {selectedUser ? (<>Editing: <strong className="text-text">{selectedUser.name}</strong> ({selectedUser.user_type.replace('_', ' ')})</>) : 'Select a client admin to configure permissions'}
