        ...
        {step === 'new' && !mastersLoading && (
          <div className="smp-body snf-body">
            ...
            <div className="snf-section">
              ...
              <div className="snf-row snf-row-1"><div className="snf-field"><label className="snf-lbl">Street Address <span className="snf-req">*</span><span style={{ float: 'right', fontSize: 10.5, fontWeight: 600, color: addr.length >= 255 ? '#ef4444' : addr.length >= 230 ? '#f59e0b' : '#94a3b8' }}>{addr.length}/255</span></label><input className="snf-inp" style={invStyle(errs.addr)} maxLength={255} value={addr} onChange={e => setAddr(e.target.value)} placeholder="e.g. 101, Business Park, MG Road" />{errMsg(errs.addr)}</div></div>
