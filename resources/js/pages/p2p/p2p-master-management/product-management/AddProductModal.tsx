...
export default function AddProductModal(props: {
 ...
 const REMARKS_MIN = 3;
  ...
  /* Un-map a supplier from this product. Reuses commitVendorList so it behaves
   * exactly like edit/add: optimistic list update, auto-persist to the server
   * when the product already exists (persistsImmediately), rollback + toast on
   * failure. If the row being edited is the one removed, drop the edit draft. */
  const removeVendor = async (v: VendorEntry) => {
    if (vendorEditingId === v.id) closeVendorDraft(); // drop the edit draft for the removed row
    await commitVendorList(
      vendors.filter(x => x.id !== v.id),
      'Supplier removed',
      `${v.vendorName || 'Supplier'} has been unmapped from this product.`,
    );
  };
