...
type VaultRow = {
  ...
  date: string; amount: number | null; pill: ReactNode; file: string;
};
...
export default function TxnVaultModal({ detail, onClose }: { detail: PaymentRequestDetail; onClose: () => void }) {
  ...
  const poRows: VaultRow[] = po ? [{
    ...
    file: poDoc?.attachment ?? `${po.po.replace(/[^0-9A-Za-z]+/g, '_')}.pdf`,
  }] : [];
  const tdRows: VaultRow[] = tradeDocs.filter(d => d !== poDoc).map(d => ({
    ...
    date: d.generated, amount: null, pill: signPill(d.status), file: d.attachment,
  }));
  ...
  const spiRows: VaultRow[] = po
    ? po.invoices.map(l => ({
      ...
      file: `${l.spi.replace(/[^0-9A-Za-z]+/g, '_')}.pdf`,
    }))
    : [{
      ...
      file: `${doc.id.replace(/[^0-9A-Za-z]+/g, '_')}.pdf`,
    }];
        ...
        <div className="ptv-tbl">
          ...
          {rows.map(r => (
            <div className="ptv-row" key={r.id}>
              ...
              <span className="ptv-c ptv-c--name"><span className="ptv-nm">{r.name}</span><span className="ptv-nmsub" title={r.nameSub}>{r.nameSub}</span></span>
              ...
              <span className="ptv-c ptv-c--act">
                <button type="button" className="ptv-btn ptv-btn--view" title={`View ${r.file}`} onClick={() => soon(`Opening ${r.file}`)}><IcoEye size={14} /></button>
                <button type="button" className="ptv-btn ptv-btn--dl" title={`Download ${r.file}`} onClick={() => soon(`Downloading ${r.file}`)}><IcoDownload size={14} /></button>
              </span>
            </div>
          ))}
        </div>
    ...
    <div className="prd-vault" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="ptv-box" role="dialog" aria-modal="true" aria-label="Evidence Vault">
        ...
        <div className="ptv-ft">
          ...
          <button type="button" className="ptv-all" onClick={() => soon(`Downloading all ${total} documents`)}><IcoDownload size={14} /><span>Download All</span></button>
        </div>
      </div>
    </div>,
    document.body,
