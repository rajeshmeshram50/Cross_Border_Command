# COMPLIANCE BEHAVIOURS MASTER — CODE WALKTHROUGH
...
All behaviour is in `app/Http/Controllers/Api/MasterController.php`, driven by the `compliance_behaviours` schema (uFields `name`). No dedicated controller. Read scope in `MasterVisibility.php`.
...
3. `?search=` → ILIKE OR across `name`, `action_required`, `status`.
...
2. `validatePayload()` — `name` required ≤50, `action_required` nullable, `status` enum; `uFields=[name]` single-text → case-insensitive uniqueness.
...
|---|---|---|
| Permission | `authorizeMaster` | `master.compliance_behaviours`; super_admin bypass |
...
- `action_required` is free-text guidance — no workflow trigger.
- `next-code` → `{code:null}`.

---
*Related documents: COMPLIANCE_BEHAVIOURS_FUNCTIONAL_DOCUMENTATION.md, COMPLIANCE_BEHAVIOURS_TECHNICAL_DOCUMENTATION.md, COMPLIANCE_BEHAVIOURS_API_DOCUMENTATION.md*
